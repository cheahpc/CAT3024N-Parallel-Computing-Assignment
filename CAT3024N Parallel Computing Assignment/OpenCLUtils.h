...
bool AddSources(cl::Program::Sources &sources, const string &file_name)
{
...
int getDeviceSize(int platform_id)
{
	vector<cl::Platform> platforms;
	cl::Platform::get(&platforms);
	vector<cl::Device> devices;
...
string ListPlatformsDevices()
{
	...
	{
		sstream << "Platform " << i << " | " << platforms[i].getInfo<CL_PLATFORM_NAME>() << ", OpenCL Version: " << platforms[i].getInfo<CL_PLATFORM_VERSION>() << endl;
		...
		{
			sstream << endl;
			sstream << "\tDevice " << j << " | " << devices[j].getInfo<CL_DEVICE_NAME>() << endl;
			sstream << "\t\tDriver Version: " << devices[j].getInfo<CL_DRIVER_VERSION>() << endl;
			sstream << "\t\tOpenCL Version: " << devices[j].getInfo<CL_DEVICE_VERSION>() << endl;
			sstream << "\t\tVendor: " << devices[j].getInfo<CL_DEVICE_VENDOR>() << endl;

			cl_device_type device_type = devices[j].getInfo<CL_DEVICE_TYPE>();
			sstream << "\t\tType: ";
			if (device_type & CL_DEVICE_TYPE_DEFAULT)
				sstream << "DEFAULT" << endl;
			if (device_type & CL_DEVICE_TYPE_CPU)
				sstream << "CPU" << endl;
			if (device_type & CL_DEVICE_TYPE_GPU)
				sstream << "GPU" << endl;
			if (device_type & CL_DEVICE_TYPE_ACCELERATOR)
				sstream << "ACCELERATOR " << endl;
			sstream << "\t\tCompute units: " << devices[j].getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>() << endl;
			sstream << "\t\tClock freq [MHz]: " << devices[j].getInfo<CL_DEVICE_MAX_CLOCK_FREQUENCY>() << endl;
			sstream << "\t\tMax memory size [B]: " << devices[j].getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() << endl;
			sstream << "\t\tMax allocatable memory [B]: " << devices[j].getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>() << endl;
			sstream << "\t\tLocal memory size [B]: " << devices[j].getInfo<CL_DEVICE_LOCAL_MEM_SIZE>() << endl;
			sstream << "\t\tMax work group size: " << devices[j].getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>() << endl;
			sstream << "\t\tMax work item dimensions: " << devices[j].getInfo<CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS>() << endl;
			sstream << "\t\tMax work item sizes: " << devices[j].getInfo<CL_DEVICE_MAX_WORK_ITEM_SIZES>() << endl;
			sstream << endl;
		}
	}
	return sstream.str();
